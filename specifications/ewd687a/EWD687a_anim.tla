...
Legend ≜
    ...
    *)
    LET BasePos ≜ [ x ↦ 350, y ↦ 120 ]
        ...
        Action ≜ TLCGet("action").name ∘ ToString(TLCGet("action").context) IN
    Group(⟨Text(BasePos.x, BasePos.y, "Step: " ∘ ToString(TLCGet("level")), ⟨⟩),
            Text(BasePos.x, BasePos.y + 20, "A:  " ∘ Action, ⟨⟩),
            Text(BasePos.x, BasePos.y + 40, "A': " ∘ Action', ⟨⟩),
    ...
    LET ES[ e ∈ Edges ] ≜
        LET from ≜ GraphNodePos[e[1]] to ≜ GraphNodePos[e[2]]
            ...
            messages ≜ Text(mpt.x, mpt.y, 
                Repeat("m", msgs[e]) ∘ Repeat("a", acks[e]), ⟨⟩)
...
U ≜ 
    ...
    LET UE[ u ∈ {upEdge[p] : p ∈ DOMAIN upEdge} \ {NotAnEdge} ] ≜
...
Frame ≜
    ...
    SVGDefs ∘ SVGElemToString(Group(⟨Legend, E, U, N⟩, ⟨⟩))
...
Alias ≜ [
    ...
    file ↦ 
        ...
        ToFile("<svg viewBox='0 0 1400 1200'>" ∘ Frame ∘ "</svg>", "EWD687a_anim_", TLCGet("level"), 3, ".svg")
    ]
