...
NoNode ≜ CHOOSE x : x ∉ Node
neighbors(n) ≜ { m ∈ Node : R[m,n] }

(**
--algorithm Echo {
    variable inbox = [n ∈ Node ↦ {}];  \* model communication between nodes
    define { \* sending and receiving messages
      \* network obtained from net when p sends message of kind knd to q
      send(net, p, q, knd) ≜ [net EXCEPT ![q] = @ ∪ {[kind ↦ knd, sndr ↦ p]}]
      ...
      multicast(net, p, dest, knd) ≜
        [m ∈ Node ↦ IF m ∈ dest THEN net[m] ∪ {[kind ↦ knd, sndr ↦ p]}
                        ELSE net[m]]
    }
  ...
  n1: while (rcvd < Cardinality(nbrs)) {
         ...
         with (msg ∈ inbox[self],
            ...
            if (msg.kind = "c") { children ≔ children ∪ {msg.sndr} }
         }  \* end with
      }; \* end while
...
Init ≜ (* Global variables *)
        ∧ inbox = [n ∈ Node ↦ {}]
        (* Process node *)
        ∧ parent = [self ∈ Node ↦ NoNode]
        ∧ children = [self ∈ Node ↦ {}]
        ∧ rcvd = [self ∈ Node ↦ 0]
        ∧ nbrs = [self ∈ Node ↦ neighbors(self)]
        ∧ pc = [self ∈ ProcSet ↦ "n0"]
                            ...
                            LET net ≜ receive(inbox, self, msg) IN
                              ...
                              ∧ IF self ≠ initiator ∧ rcvd'[self] = 1
                                    ...
                                    ELSE ∧ inbox' = net
                                         ∧ UNCHANGED parent
                              ∧ IF msg.kind = "c"
                                    THEN ∧ children' = [children EXCEPT ![self] = children[self] ∪ {msg.sndr}]
                                    ELSE ∧ TRUE
                                         ∧ UNCHANGED children
                       ∧ pc' = [pc EXCEPT ![self] = "n1"]
...
n2(self) ≜ ∧ pc[self] = "n2"
           ∧ IF self ≠ initiator
                  THEN ∧ Assert((parent[self] ∈ nbrs[self]), 
...
(***************************************************************************)
TypeOK ≜
  ∧ parent ∈ [Node → (Node ∪ {NoNode})]
  ∧ children ∈ [Node → SUBSET Node]
  ∧ rcvd ∈ [Node → ℕ]
  ∧ nbrs ∈ [Node → SUBSET Node]
  ∧ ∀ n ∈ Node : nbrs[n] = neighbors(n) ∧ rcvd[n] ≤ Cardinality(nbrs[n])
  ∧ inbox ∈ [Node → SUBSET [kind : {"m","c"}, sndr : Node]]
  ∧ ∀ n ∈ Node : ∀ msg ∈ inbox[n] : msg.sndr ∈ nbrs[n]
...
ParentIsNeighbor ≜ ∀ n ∈ Node : parent[n] ∈ neighbors(n) ∪ {NoNode}

(* A node n is a child of node m only if m is the parent of n.
   At the end of the computation, this is "if and only if". *)
ParentChild ≜ ∀ m,n ∈ Node :
  ∧ n ∈ children[m] ⇒ m = parent[n]
  ∧ m = parent[n] ∧ pc[m] = "Done" ⇒ n ∈ children[m]

(* Compute the ancestor relation *)
IsParent ≜ [m,n ∈ Node ↦ n = parent[m]]
...
AncestorProperties ≜
  (∀ n ∈ Node : pc[n] = "Done")
  ⇒ LET anc ≜ IsAncestor
     IN  ∧ ∀ n ∈ Node \ {initiator} : anc[n, initiator]
