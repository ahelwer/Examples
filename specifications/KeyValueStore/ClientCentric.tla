...
TimeStamp ≜ ℕ
TransactionTimes ≜ [t ∈ Transaction ↦ [start: TimeStamp, commit: TimeStamp]]
...
\* Seq
executionStates(execution) ≜ [ i ∈ 1‥Len(execution) ↦ execution[i].parentState ]
\* Set
executionTransactions(execution) ≜ { ep.transaction : ep ∈ SeqToSet(execution) }
...
parentState(execution, transaction) ≜ 
  LET ind ≜ CHOOSE i ∈ 1‥Len(execution): execution[i].transaction = transaction
...
Preread(execution, transaction) ≜
  ∀ operation ∈ SeqToSet(transaction): ReadStates(execution, operation, transaction) ≠ {}

PrereadAll(execution, transactions) ≜ 
  ∀ transaction ∈ transactions: Preread(execution, transaction)
...
Complete(execution, transaction, state) ≜ 
  LET setOfAllReadStatesOfOperation(transactionOperations) ≜
        { ReadStates(execution, operation, transaction) : operation ∈ SeqToSet(transactionOperations) }
     \* readStatesForEmptyTransaction contains all previous states, to ensure that empty txns do not incorrectly invalidate the checked isolation level
      readStatesForEmptyTransaction ≜ { s ∈ SeqToSet(executionStates(execution)) : beforeOrEqualInExecution(execution, s, parentState(execution, transaction)) }
  IN state ∈ INTERSECTION(setOfAllReadStatesOfOperation(transaction) ∪ { readStatesForEmptyTransaction } )
...
WriteSet(transaction) ≜ 
  LET writes ≜ { operation ∈ SeqToSet(transaction) : operation.op = "write" } 
  IN { operation.key : operation ∈ writes } 
...
NoConf(execution, transaction, state) ≜ 
  LET Sp ≜ parentState(execution, transaction)
      delta ≜ { key ∈ DOMAIN Sp : Sp[key] ≠ state[key] }
  IN delta ∩ WriteSet(transaction) = {}
...
executionSatisfiesCT(execution, commitTest(_,_)) ≜
  ...
  IN ∀ transaction ∈ transactions: commitTest(transaction, execution)
...
satisfyIsolationLevel(initialState, transactions, commitTest(_,_)) ≜
  ∃ execution ∈ executions(initialState, transactions): ∀ transaction ∈ transactions:
...
\* Snapshot Isolation
CT_SI(transaction, execution) ≜ ∃ state ∈ SeqToSet(executionStates(execution)):
...
CT_SSER(timestamps, transaction, execution) ≜
  ...
  IN ∧ Complete(execution, transaction, Sp)
     ∧ ∀ otherTransaction ∈ executionTransactions(execution): 
...
StrictSerializability(initialState, transactions, timestamps) ≜
  ∃ execution ∈ executions(initialState, transactions): ∀ transaction ∈ transactions: CT_SSER(timestamps, transaction, execution)
