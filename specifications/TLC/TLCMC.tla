...
(***************************************************************************)
SetToSeqs(set, Filter(_)) ≜ UNION {{perm ∈ [1‥Cardinality(set) → set]: 
...
IsGraph(G) ≜ ∧ {"states", "initials", "actions"} = DOMAIN G
             ∧ G.actions ∈ [G.states → SUBSET G.states]
             ∧ G.initials ⊆ G.states
...
(***************************************************************************)
SuccessorsOf(state, SG) ≜ {successor ∈ SG.actions[state]:
                            successor ∉ {state}}
...
ASSUME \* The given StateGraph is actually a graph
       ...
       ∨ ViolationStates ⊆ StateGraph.states
...
--fair algorithm ModelChecker {
    variables
              ...
              \* of initial states.
              S ∈ SetOfAllPermutationsOfInitials(StateGraph),
              ...
              T = ⟨⟩;
    {
       ...
       trc : while (TRUE) {
                if (Head(counterexample) ∉ StateGraph.initials) {
                   counterexample ≔ ⟨Predecessor(T, Head(counterexample))⟩ ∘ counterexample;
                } else {
...
trc ≜ ∧ pc = "trc"
      ∧ IF Head(counterexample) ∉ StateGraph.initials
             THEN ∧ counterexample' = ⟨Predecessor(T, Head(counterexample))⟩ ∘ counterexample
                  ∧ pc' = "trc"
...
Live ≜ ViolationStates ≠ {} ⇒ ◇□(∧ Len(counterexample) > 0
                                 ∧ counterexample[Len(counterexample)] ∈ ViolationStates
                                 ∧ counterexample[1] ∈ StateGraph.initials)
