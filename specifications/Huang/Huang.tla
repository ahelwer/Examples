...
TypeOK ≜
    ...
    ∧ ∀ p ∈ Procs : IsDyadicRational(weight[p])
...
Init ≜
    ...
    ∧ active = [ p ∈ Procs ↦ IF p = Leader THEN TRUE ELSE FALSE ]
    \* No weights have been "thrown" yet.
    ∧ weight = [ p ∈ Procs ↦ IF p = Leader THEN One ELSE Zero ]
...
Next ≜
    ∨ ∃ p ∈ Procs : SendMsg(p)
    ∨ ∃ p ∈ Procs \ {Leader} : RcvMsg(p) ∨ Idle(p)
    ∨ RcvLdr ∨ IdleLdr
...
Spec ≜
        ...
        ∧ WF_vars(RcvLdr)
        ∧ WF_vars(IdleLdr)
        ∧ ∀ p ∈ Procs \ {Leader}: WF_vars(RcvMsg(p))
        ∧ ∀ p ∈ Procs \ {Leader}: WF_vars(Idle(p))
...
StateConstraint ≜
    ...
    ∀ p ∈ Procs : weight[p].den ≤ 8

Alias ≜
    [
        \* Pretty printing of error-trace states.
        active ↦ {p ∈ Procs : active[p]},
        weights ↦ [p ∈ { q ∈ Procs: weight[q] ≠ Zero} 
                        ↦ PrettyPrint(weight[p])],
        msgs ↦ [p ∈ { q ∈ Procs: msgs[q] ≠ ⟨⟩} 
                        ↦ [ i ∈ 1‥Len(msgs[p]) ↦ PrettyPrint(msgs[p][i])]]
    ]
