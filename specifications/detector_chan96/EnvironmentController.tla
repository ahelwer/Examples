...
K ≜ ((SendPoint * PredictPoint * PHI) ÷ DELTA) + 1 
...
Init ≜
  ∧ CommChan!Init
  ∧ Detector!Init
  ∧ procPause = [ i ∈ Proc ↦ 0 ]   (* No pauses *)
  ∧ moved = [ i ∈ Proc ↦ "INIT" ]  (* Every process finishes the initialization. *)                                         
  ∧ failed = [ i ∈ Proc ↦ FALSE ]  (* No failures *)    
  ∧ F = 0                              (* No failures *)
...
SomeLocallyTicked ≜
  ∃ p ∈ Proc : moved[p] ≠ "CRASH" ∧ moved[p] ≠ "NO"             
...
EnvTick ≜
  ...
  ∧ CommChan!AttainAge
  ∧ moved' = [ i ∈ Proc ↦ "NO" ]
  ∧ procPause' = [ i ∈ Proc ↦ IF failed[i] = FALSE 
                                   THEN procPause[i] + 1
                                   ELSE -1 ]
  ∧ UNCHANGED ⟨ failed, F ⟩
  ∧ UNCHANGED procVars      
...
OnlyMessagesForCorrectProcesses(msgs) ≜  { m ∈ msgs : failed[m.to] = FALSE }          
...
(* Two constraints PHIConstraint and DELTAConstraint are respectively restrictions on
   ...
   are allowed in the computation model of partial synchrony.  
     ...
     - DELTAConstraint: If message m is placed in p_j’s buffer by some Send(m, p_j)  
       ...
       are DELTA. *)                                                                             
PHIConstraint ≜
  ∀ i ∈ Proc : (failed'[i] = FALSE ⇒ procPause'[i] ≤ PHI)  (* No errors happen *)
...
PHIConstraint1 ≜
  ∀ i ∈ Proc : (failed'[i] = TRUE ∨ procPause'[i] ≤ PHI)   
  

                                                                                                                                                                         
DELTAConstraint ≜
  ∀ i ∈ Proc : ∧ ((failed'[i] = FALSE ∧ moved'[i] = "RECEIVE") 
                          ⇒ ( ∀ m ∈ inTransit' : ∨ m.content.to ≠ i
                                                 ∨ m.age ≤ DELTA ))
                                                     (*
                  /\ ((failed'[i] = TRUE) 
...
TypeOK ≜
  ∧ CommChan!TypeOK
  ∧ Detector!TypeOK       
  ∧ procPause ∈ [ Proc → { -1 } ∪ 0‥PHI ]
  ∧ ∀ i ∈ Proc : failed[i] = TRUE ⇔ procPause[i] = -1 
  ∧ failed ∈ [ Proc → BOOLEAN ]
  ∧ moved ∈ [ Proc → Actions]
  ∧ F ∈ 0‥T      
  ∧ ∀ i, j ∈ Proc : 0 ≤ delta[i][j] ∧ delta[i][j] ≤ maxDelta
  ∧ ∀ box ∈ inTransit : 0 ≤ box.age ∧ box.age ≤ maxAge   
  ∧ ∀ i, j ∈ Proc : fromLastHeard[i][j] ≤ maxAge
...
(*Two properties of an eventually perfect failure detector 
    ...
    - Eventually Strong Accuracy: There is a time after which correct processes 
      are not suspected by any correct process. *) 
StrongCompleteness ≜   
  ◇□ ( ∀ j ∈ Proc : failed[j] = TRUE ⇒ (∀ i ∈ Proc : ∨ failed[i] = TRUE 
                                                     ∨ j ∈ suspected[i] ) )
                                                             
EventuallyStrongAccuracy ≜     
  ◇□ ( ∀ i, j ∈ Proc : (failed[i] = FALSE ∧ failed[j] = FALSE)  
                                ⇒ j ∉ suspected[i] )     
