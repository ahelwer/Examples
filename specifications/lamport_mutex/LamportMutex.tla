...
ASSUME NType ≜ N ∈ ℕ
ASSUME maxClockType ≜ maxClock ∈ ℕ
...
Message ≜ {AckMessage, RelMessage} ∪ {ReqMessage(c) : c ∈ Clock}
...
(***************************************************************************)
Broadcast(s, m) ≜
  [r ∈ Proc ↦ IF s=r THEN network[s][r] ELSE Append(network[s][r], m)]
...
Next ≜
  ∨ ∃ p ∈ Proc : Request(p) ∨ Enter(p) ∨ Exit(p)
  ∨ ∃ p ∈ Proc : ∃ q ∈ Proc \ {p} : 
...
(***************************************************************************)
ClockConstraint ≜ ∀ p ∈ Proc : clock[p] ≤ maxClock
...
(***************************************************************************)
BoundedNetwork ≜ ∀ p,q ∈ Proc : Len(network[p][q]) ≤ 3
