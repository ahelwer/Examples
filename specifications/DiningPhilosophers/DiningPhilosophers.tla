...
Init ≜ (* Global variables *)
        ...
        (* Process Philosopher *)
        ∧ hungry = [self ∈ 1‥NP ↦ TRUE]
        ∧ pc = [self ∈ ProcSet ↦ "Loop"]
...
Next ≜ (∃ self ∈ 1‥NP: Philosopher(self))

Spec ≜ ∧ Init ∧ □[Next]_vars
       ∧ ∀ self ∈ 1‥NP : WF_vars(Philosopher(self))
...
ShareFork(p, q) ≜
    {LeftFork(p), RightFork(p)} ∩ {LeftFork(q), RightFork(q)} ≠ {}
...
*)
TypeOK ≜
    ∧ forks ∈ [1‥NP → [holder: 1‥NP, clean: BOOLEAN]]
    ∧ hungry ∈ [1‥NP → BOOLEAN]
    ∧ pc ∈ [1‥NP → {"Loop", "Eat", "Think"}]
