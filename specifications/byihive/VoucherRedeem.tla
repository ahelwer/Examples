...
VTPTypeOK ≜
  ...
  (*************************************************************************)
  ∧ vState ∈ [V → {"valid", "redeemed"}]
  ∧ vlcState ∈ [V → {"working", "done"}]
  ∧ hState ∈ [H → {"holding", "prepared", "redeemed", "aborted"}]
  ∧ cState ∈ [C → {"waiting", "prepared", "redeemed", "aborted"}]
  ∧ vtpState ∈ {"init", "done"}
  ∧ vtpRPrepared ⊆ (H ∪ C)
  ∧ msgs ⊆ Messages
...
VTPRcvPrepared(h,c) ≜
  ...
  ∧ [type ↦ "Prepared", vc ↦ c] ∈ msgs
  ∧ vtpRPrepared' = vtpRPrepared ∪ {h,c}
