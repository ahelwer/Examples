...
moreNplus3Tdiv2 ≜ (N + 3 * T) ÷ 2 + 1
moreNminusTdiv2 ≜ (N - T) ÷ 2 + 1

VARIABLE pc, rcvd, sent

ASSUME  ∧ N ∈ ℕ ∧ T ∈ ℕ ∧ F ∈ ℕ
        ∧ moreNplus3Tdiv2 ∈ ℕ ∧ moreNminusTdiv2 ∈ ℕ
...
rcvd0(self) ≜ Cardinality({m ∈ rcvd'[self] : m[2] = "ECHO0"})         
  
rcvd1(self) ≜ Cardinality({m ∈ rcvd'[self] : m[2] = "ECHO1"})  
 ...
 *)
rcvd01(self) ≜ Cardinality({p ∈ P : (∃ m ∈ rcvd'[self] : m[1] = p)})       
...
(* Initial step *)
Init ≜
  ∧ pc ∈ [ Corr → {"V0", "V1"} ]  (* Processes can propose V0 and V1. *) 
  ∧ sent = {}                        (* No message has sent. *)     
  ∧ rcvd = [ i ∈ Corr ↦ {} ]     (* No message has received. *)
...
TypeOK ≜ 
  ∧ sent ⊆ P × M
  ∧ pc ∈ [ Corr → {"V0", "V1", "S0", "S1", "D0", "D1", "U0", "U1"} ]
  ∧ rcvd ∈ [ Corr → SUBSET (P × M) ]
          
Lemma3_0 ≜ (∃ self ∈ Corr: (pc[self] = "D0")) ⇒ (∀ self ∈ Corr: (pc[self] ≠ "D1"))  
Lemma3_1 ≜ (∃ self ∈ Corr: (pc[self] = "D1")) ⇒ (∀ self ∈ Corr: (pc[self] ≠ "D0"))  

Lemma4_0 ≜ (∃ self ∈ Corr: (pc[self] = "D0")) ⇒ (∀ self ∈ Corr: (pc[self] ≠ "U1"))  
Lemma4_1 ≜ (∃ self ∈ Corr: (pc[self] = "D1")) ⇒ (∀ self ∈ Corr: (pc[self] ≠ "U0"))  
