...
TypeOK ≜
    ∧ tcb ∈ [ Peers → BOOLEAN ]
    ∧ connstate ∈ [ Peers → States ]
    ∧ network ∈ [ Peers → Seq({"SYN", "SYN,ACK", "ACK", "RST", "FIN"} ∪ {"ACKofFIN"}) ]

Init ≜
    ∧ tcb = [p ∈ Peers ↦ FALSE]
    ∧ connstate = [p ∈ Peers ↦ "CLOSED"]
    ∧ network = [p ∈ Peers ↦ ⟨⟩]
...
Spec ≜
    ...
    ∧ WF_vars(∃ local, remote ∈ Peers: CLOSE_SYN_SENT(local, remote))
...
Inv ≜
    ...
    ∀ local, remote ∈ { p ∈ Peers : network[p] = ⟨⟩ } :
        connstate[local] = "ESTABLISHED" ⇔ connstate[remote] = "ESTABLISHED"

Prop ≜
    ∀ p ∈ Peers :
        connstate[p] = "SYN-SENT" ↝ connstate[p] ∈ {"ESTABLISHED", "LISTEN", "CLOSED"}
