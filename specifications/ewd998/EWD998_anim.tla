...
Labels ≜ Group(⟨Text(LegendBasePos.x, LegendBasePos.y, "Circle: Active, Black: Tainted", Arial),
                  ...
                  Text(LegendBasePos.x, LegendBasePos.y + 60, "Level: " ∘ ToString(TLCGet("level")) ∘ 
                               " Terminated: " ∘ (IF EWD998Chan!EWD998!Termination THEN "T" ELSE "F") ∘
                               " Detected: " ∘ (IF EWD998Chan!EWD998!terminationDetected THEN "T" ELSE "F"), Arial)⟩,
                  ⟨⟩)
    ...
    LET RN[ n ∈ Node ] ≜         
            LET coord ≜ NodeOfRingNetwork(RingBasePos.w, RingBasePos.h, RingBasePos.r, node2nat[n], N)    
                id ≜ Text(coord.x + ArrowPosOffset + 2, coord.y + ArrowPosOffset + 7, ToString(counter[n]), 
                                                Arial @@ [fill ↦ CHOOSE c ∈ Color : c ≠ color[n]])
...
TokenNetwork ≜     
    LET coord ≜ NodeOfRingNetwork(TokenBasePos.w, TokenBasePos.h, TokenBasePos.r, tpos, N)    
        ...
        id    ≜ Text(coord.x - 5, coord.y + 3, ToString(token.q), [font ↦ "Arial", fill ↦ CHOOSE c ∈ Color : c ≠ token.color ])
            ...
            I[ pl ∈ pls ] ≜
                LET from ≜ NodeOfRingNetwork(RingBasePos.w, RingBasePos.h, RingBasePos.r, node2nat[pl.src], N)
                    ...
                    line ≜ Line(from.x + ArrowPosOffset, from.y + ArrowPosOffset, 
                                    ...
                                    [stroke ↦ "orange", stroke_dasharray ↦ IF pl ∈ plsN THEN "5" ELSE "0",
    ...
    IOExecTemplate(
            ⟨"bash", "-c", "echo \"%1$s\" > EWD998_anim.svg"⟩,
                ⟨"<svg viewBox='-40 15 330 290'>"
                    ∘ Defs
                    ∘ Animation'  \* Note the prime here!
                ⟩).exitValue = 0

Alias ≜ [ 
    toolbox ↦ Defs ∘ Animation,
...
AnimSpec ≜
    ...
    ∧ active = [ n ∈ Node ↦ FALSE ]
    ∧ color = [ n ∈ Node ↦ "black" ]
    ∧ counter = [n ∈ Node ↦ IF Cardinality(Node) % 2 = 0
                                 ...
                                 ELSE IF n ≠ Initiator 
                                      THEN IF node2nat[n] % 2 = 0
                                           ...
                                           ELSE -1
                                      ELSE 0 ]
    ∧ inbox = [n ∈ Node ↦ IF n  = Initiator \* RingOfNodes[Initiator] 
                              THEN ⟨ [type ↦ "tok", q ↦ 0, color ↦ "black" ] ⟩ 
                              ELSE ⟨⟩] \* with empty channels.
    ∧ Init!5
    ∧ □[∃ n ∈ Node: System(n)]_vars
