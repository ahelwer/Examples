...
--algorithm ewd998 {
  ...
  variables
    ...
    *)
    network = [n ∈ Node ↦ IF n = Initiator THEN SetToBag({[type↦ "tok", q ↦ 0, color ↦ "black"]}) ELSE EmptyBag];
  ...
  fair process (node ∈ Node) 
    variables active ∈ BOOLEAN, color = "black", counter = 0;
  {
l:  while (TRUE) {
      ...
      either { \* send some payload message to some other node.
        ...
        with (msg ∈ pendingMsgs(network, self)) {
            when msg.type = "pl";
            counter ≔ counter - 1;
            active ≔ TRUE;
            color ≔ "black";
        ...
        when self ≠ Initiator;
        with (tok ∈ pendingMsgs(network, self)) {
...
token ≜
    LET tpos ≜ CHOOSE i ∈ Node : ∃ m ∈ DOMAIN network[i]: m.type = "tok"
        tok ≜ CHOOSE m ∈ DOMAIN network[tpos] : m.type = "tok"
    IN [pos ↦ tpos, q ↦ tok.q, color ↦ tok.color]

pending ≜
    [n ∈ Node ↦ IF [type↦"pl"] ∈ DOMAIN network[n] THEN network[n][[type↦"pl"]] ELSE 0]
