...
Merge(n, r, l) ≜
    ...
    IN [ m ∈ DOMAIN l ↦ IF m = n THEN l[m] + 1 ELSE max(r[m], l[m]) ]
...
Initiator ≜ CHOOSE n ∈ Node : TRUE
                                         
\* Organize Nodes in a ring. 
RingOfNodes ≜ 
  CHOOSE r ∈ [ Node → Node ]: IsSimpleCycle(Node, r)
...
\* Worst-case WRT Max3TokenRounds, token is at node N-2.
  ∧ inbox = [n ∈ Node ↦ IF n = nat2node[N-2] 
                              THEN ⟨ [type ↦ "tok", q ↦ 0, color ↦ "black", vc ↦ clock[n] ] ⟩ 
                              ELSE ⟨⟩] \* with empty channels.
  (* EWD840 *) 
  ∧ active ∈ [Node → {FALSE}]
...
Spec ≜ Init ∧ □[∃ n ∈ Node: Next(n)]_vars
             ∧ ∀ n ∈ Node: WF_vars(System(n))
...
Node2Nat(fcn) ≜
  [ i ∈ 0‥N-1 ↦  fcn[nat2node[i]] ]

MapSeq(seq, Op(_)) ≜
    LET F[i ∈ 0‥Len(seq)] ≜ IF i = 0
                                THEN ⟨ ⟩
