...
PassTokenOpts(n) ≜
  ...
  ∧ ∨ ¬ active[n] \* If machine i is active, keep the token.
    ∨ ∧ "pt1" ∈ F
      ∧ color[n] = "black"
    ∨ ∧ "pt2" ∈ F
      ∧ ∃ j ∈ 1‥Len(inbox[n]) : inbox[n][j].type = "tok" ∧ inbox[n][j].color = "black"
  ∧ ∃ j ∈ 1‥Len(inbox[n]) : 
          ∧ inbox[n][j].type = "tok"
          \* the machine nr.i+1 transmits the token to machine nr.i under q := q + c[i+1]
          ∧ LET tkn ≜ inbox[n][j]
             IN  inbox' = [inbox EXCEPT ![CASE "pt3" ∈ F ∧ color[n] = "black" → 0
                                            □ "pt4" ∈ F ∧ tkn.color ="black" → 0
                                            □ OTHER    →  n-1] = 
...
SpecOpts ≜ InitSim ∧ Init ∧ □[∃ n ∈ Node: SystemOpts(n) ∨ Environment]_vars
...
Nodes ≜
    CHOOSE n ∈ 1‥256 : ToString(n) = IOEnv.N
