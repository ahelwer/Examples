...
LEMMA FoldFunctionOnSetIterate ≜
  ASSUME NEW op(_,_), 
         ...
         NEW base ∈ T, NEW fun ∈ [S → T], 
         NEW inds ∈ SUBSET S, NEW e ∈ inds,
...
LEMMA FoldFunctionOnSetUnion ≜
  ASSUME NEW op(_,_),
         NEW S, IsFiniteSet(S), NEW T,
         NEW base ∈ T, NEW fun ∈ [S → T],
         NEW inds1 ∈ SUBSET S, NEW inds2 ∈ SUBSET S, inds1 ∩ inds2 = {},
         IsAssociativeOn(op, T), IsCommutativeOn(op, T), IsIdentityOn(op, base, T)
  PROVE  FoldFunctionOnSet(op, base, fun, inds1 ∪ inds2)
         = op(FoldFunctionOnSet(op, base, fun, inds1), FoldFunctionOnSet(op, base, fun, inds2))

LEMMA FoldFunctionOnSetEqual ≜
  ASSUME NEW op(_,_),
         NEW S, IsFiniteSet(S), NEW T, NEW base ∈ T,
         NEW f ∈ [S → T], NEW g ∈ [S → T],
         NEW inds ∈ SUBSET S,
         ∀ x ∈ inds : f[x] = g[x]
...
LEMMA FoldFunctionOnSetType ≜ 
  ASSUME NEW op(_,_),
         ...
         NEW inds ∈ SUBSET S,
         ∀ x,y ∈ T : op(x,y) ∈ T
  PROVE  FoldFunctionOnSet(op, base, fun, inds) ∈ T
...
LEMMA SumIterate ≜
  ASSUME NEW fun ∈ [Node → ℤ], 
         NEW inds ∈ SUBSET Node, NEW e ∈ inds
  PROVE  Sum(fun, inds) = fun[e] + Sum(fun, inds \ {e})
\* BY FoldFunctionOnSetIterate, NodeIsFinite, PlusACI DEF Sum (* fails *)

LEMMA SumSingleton ≜
  ASSUME NEW fun ∈ [Node → ℤ], NEW x ∈ Node
  PROVE  Sum(fun, {x}) = fun[x]
BY SumIterate, SumEmpty, Isa

LEMMA SumUnion ≜
  ASSUME NEW fun ∈ [Node → ℤ],
         NEW inds1 ∈ SUBSET Node, NEW inds2 ∈ SUBSET Node, inds1 ∩ inds2 = {}
  PROVE  Sum(fun, inds1 ∪ inds2) = Sum(fun, inds1) + Sum(fun, inds2)

LEMMA SumEqual ≜
  ASSUME NEW f ∈ [Node → ℤ], NEW g ∈ [Node → ℤ],
         NEW inds ∈ SUBSET Node,
         ∀ x ∈ inds : f[x] = g[x]
...
LEMMA SumIsInt ≜ 
  ASSUME NEW fun ∈ [Node → ℤ],
         NEW inds ∈ SUBSET Node
  PROVE  Sum(fun, inds) ∈ ℤ
BY FoldFunctionOnSetType, NodeIsFinite, Isa DEF Sum

LEMMA SumIsNat ≜ 
  ASSUME NEW fun ∈ [Node → ℕ],
         NEW inds ∈ SUBSET Node
  PROVE  Sum(fun, inds) ∈ ℕ
BY FoldFunctionOnSetType, NodeIsFinite, Isa DEF Sum

LEMMA SumZero ≜
  ASSUME NEW fun ∈ [Node → ℤ], NEW inds ∈ SUBSET Node,
         ∀ i ∈ inds : fun[i] = 0
  PROVE  Sum(fun, inds) = 0
<1>1. IsFiniteSet(inds)
  BY NodeIsFinite, FS_Subset
<1>. DEFINE P(T) ≜ T ⊆ inds ⇒ Sum(fun, T) = 0
<1>2. P({})
  BY SumEmpty
<1>3. ASSUME NEW T, NEW x, IsFiniteSet(T), P(T), x ∉ T
      PROVE  P(T ∪ {x})
  BY <1>3, SumIterate
<1>4. P(inds)
  <2>. HIDE DEF P
  ...
  <2>2. ASSUME NEW i ∈ Node \ {0},
    ...
    <3>2. Inv!2'
      <4>1. ASSUME Inv!P1 PROVE Inv!P1'
        <5>1. ∀ j ∈ Rng(token.pos+1, N-1)' : active'[j] = FALSE
          BY <2>2, <4>1 DEF PassToken, TypeOK, Token, Node, Rng
        <5>2. token'.pos ≠ N-1
          BY <2>2 DEF PassToken, TypeOK, Token, Node
        <5>a. ∧ Rng(token.pos+1, N-1) ∈ SUBSET Node
              ∧ Rng(token'.pos+1, N-1) ∈ SUBSET Node
          BY DEF Rng
        <5>b. ∧ Sum(counter, Rng(token.pos+1, N-1)) ∈ ℤ
              ∧ Sum(counter, Rng(token.pos+1, N-1))' ∈ ℤ
        ...
        <5>3. token.q = Sum(counter, Rng(token.pos+1, N-1))
          ...
          <6>2. CASE token.pos ≠ N-1
            BY <4>1, <6>2
          <6>. QED  BY <6>1, <6>2
        <5>c. token'.pos+1 ∈ Rng(token'.pos+1, N-1)
      ...
      <4>2. ASSUME Inv!P2 PROVE Inv!P2'
        <5>1. ∧ Rng(0, token.pos) ∈ SUBSET Node
              ∧ Rng(0, token.pos)' ∈ SUBSET Node
          BY DEF Rng
        <5>2. ∧ Sum(counter, Rng(0, token.pos)) ∈ ℤ
              ∧ Sum(counter, Rng(0, token.pos))' ∈ ℤ
      ...
      <4>3. ASSUME Inv!P3 PROVE Inv!P3' ∨ Inv!P4'
        <5>1. PICK j ∈ Rng(0, token.pos) : color[j] = "black"
          BY <4>3
        <5>2. CASE j = i
          BY <2>2, <5>1, <5>2 DEF PassToken
        <5>3. CASE j ≠ i
          <6>1. j ∈ Rng(0, token'.pos)
  ...
  <2>3. ASSUME NEW i ∈ Node,
    ...
    <3>2. B' = Sum(counter, Node)'
      <4>1. B' = B + 1
        ...
        <5>2. ∀ x ∈ Node \ {j} : pending'[x] = pending[x]
          BY <3>1 DEF TypeOK
      ...
      <4>2. Sum(counter, Node)' = Sum(counter, Node) + 1
        ...
        <5>2. ∀ x ∈ Node \ {i} : counter'[x] = counter[x]
          BY <3>1 DEF TypeOK
    ...
    <3>3. ASSUME Inv!P1 PROVE Inv!P1'
      <4>1. ∧ Rng(token.pos+1, N-1) ∈ SUBSET Node
            ∧ Rng(token.pos+1, N-1)' = Rng(token.pos+1, N-1)
        BY <3>1 DEF Rng
      <4>2. ∀ x ∈ Rng(token.pos+1, N-1) : counter'[x] = counter[x]
    ...
    <3>4. ASSUME Inv!P2 PROVE Inv!P2'
      <4>1. ∧ Rng(0, token.pos) ∈ SUBSET Node
            ∧ Rng(0, token.pos)' = Rng(0, token.pos)
        BY <3>1 DEF Rng
      <4>2. CASE i ∈ Rng(0, token.pos)
        ...
        <5>2. ∀ x ∈ Rng(0, token.pos) \ {i} : counter'[x] = counter[x]
      ...
      <4>3. CASE i ∉ Rng(0, token.pos)
        <5>1. ∀ x ∈ Rng(0, token.pos) : counter'[x] = counter[x]
    ...
    <3>1. B' = Sum(counter, Node)'
      <4>1. B' = B - 1
        <5>1. ∧ B = pending[i] + Sum(pending, Node \ {i})
          ...
          BY SumIterate DEF B, TypeOK
        <5>2. ∀ x ∈ Node \ {i} : pending'[x] = pending[x]
      ...
      <4>. QED  BY <4>1, <4>2 DEF Inv
    <3>2. Inv!2'
      <4>0. i ∈ Rng(0, token.pos) ∨ i ∈ Rng(token.pos+1, N-1)
        BY DEF TypeOK, Token, Node, Rng
      <4>1. CASE i ∈ Rng(0, token.pos)
        BY <2>4, <4>1 DEF RecvMsg, TypeOK  \* node i becomes black, establishing P3'
      <4>2. CASE i ∈ Rng(token.pos+1, N-1)
        <5>0. ∧ Rng(0, token.pos) ∈ SUBSET Node
              ∧ Rng(0, token.pos)' = Rng(0, token.pos)
              ∧ Rng(token.pos+1, N-1) ∈ SUBSET Node
              ∧ Rng(token.pos+1, N-1)' = Rng(token.pos+1, N-1)
        ...
        <5>1. ASSUME Inv!P1 PROVE Inv!P2  \* then step <5>2 will show that P2 is preserved
          ...
          <6>3. CASE token.pos ≠ N-1
            ...
            <7>2. Sum(counter, Rng(0, token.pos)) + Sum(counter, Rng(token.pos+1, N-1))
                  = Sum(counter, Node)
              <8>. ∧ Rng(0, token.pos) ∪ Rng(token.pos+1, N-1) = Node
                   ∧ Rng(0, token.pos) ∩ Rng(token.pos+1, N-1) = {}
        ...
        <5>2. ASSUME Inv!P2 PROVE Inv!P2'
          <6>1. ∀ x ∈ Rng(0, token.pos) : counter'[x] = counter[x]
...
LEMMA B0NoMessagePending ≜ 
  ∧ TypeOK ∧ B=0 ⇒ ∀ i ∈ Node : pending[i] = 0
  ∧ TypeOK' ∧ B'=0 ⇒ ∀ i ∈ Node : pending'[i] = 0
<1>1. TypeOK ∧ B=0 ⇒ ∀ i ∈ Node : pending[i] = 0
  <2>. SUFFICES ASSUME TypeOK, B = 0, NEW i ∈ Node, pending[i] ≠ 0
                PROVE  FALSE
    OBVIOUS
  <2>. B = pending[i] + Sum(pending, Node \ {i})
    BY SumIterate DEF TypeOK, B
  <2>. QED  BY SumIsNat DEF TypeOK
<1>2. (TypeOK ∧ B=0 ⇒ ∀ i ∈ Node : pending[i] = 0)'
  BY <1>1, PTL
<1>. QED  BY <1>1, <1>2
...
allWhite ≜ ∀ i ∈ Node : color[i] = "white"

LEMMA Round1 ≜ BSpec ⇒ (Termination
                            ↝ Termination ∧ atMaster)
<1>. DEFINE P(n) ≜ Termination ∧ n ∈ Node ∧ token.pos = n
            Q ≜ P(0)
            R(n) ≜ BSpec ⇒ □(P(n) ⇒ ◇Q)
<1>1. ∀ n ∈ ℕ : R(n)
  <2>1. R(0)
...
<1>2. BSpec ⇒ □((∃ n ∈ ℕ : P(n)) ⇒ ◇Q)
  ...
  <2>1. BSpec ⇒ □(∀ n ∈ ℕ : P(n) ⇒ ◇Q)
    BY <1>1
  <2>2. (∀ n ∈ ℕ : P(n) ⇒ ◇Q) ⇔ ((∃ n ∈ ℕ : P(n)) ⇒ ◇Q)
    OBVIOUS
  <2>. QED  BY <2>1, <2>2, PTL
<1>3. TypeOK ⇒ (Termination ⇒ ∃ n ∈ ℕ : P(n))
...
<1>. DEFINE P(n) ≜ ∧ Termination ∧ n ∈ Node ∧ token.pos = n
                   ∧ color[0] = "white"
                   ∧ ∀ i ∈ n+1 ‥ N-1 : color[i] = "white"
            Q ≜ P(0)
            R(n) ≜ BSpec ⇒ □(P(n) ⇒ ◇Q)
<1>1. BSpec ⇒ (Termination ∧ atMaster ↝ ∃ n ∈ ℕ : P(n))
...
<1>. DEFINE P(n) ≜ ∧ Termination ∧ n ∈ Node ∧ token.pos = n
            ...
            Q ≜ P(0)
            R(n) ≜ BSpec ⇒ □(P(n) ⇒ ◇Q)
<1>1. BSpec ⇒ (Termination ∧ atMaster ∧ allWhite ↝ ∃ n ∈ ℕ : P(n))
...
<1>2. ∀ n ∈ ℕ : R(n)
  ...
  <2>2. ASSUME NEW n ∈ ℕ
    ...
    <3>0. TypeOK ∧ Pn1 ∧ System ⇒ Pn'
      ...
      <4>2. ASSUME TypeOK, Pn1, NEW i ∈ Node \ {0}, PassToken(i)
            PROVE  Pn'
...
<1>4. □TypeOK ∧ □Inv ∧ □[Next]_vars ∧ WF_vars(System) 
  ...
  <2>2. TypeOK ∧ ENABLED ⟨TD!DetectTermination⟩_(TD!vars) 
    ...
    <3>2. ∀ n ∈ Node : ¬ active[n] ∧ pending[n] = 0
