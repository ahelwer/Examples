...
Init ≜  
  ...
  ∧ nRcvdYes = [ i ∈ Proc ↦ 0 ]
  ∧ nRcvdNo = [ i ∈ Proc ↦ 0 ]  
  ∧ pc ∈ [ Proc → { "NO", "YES" } ] (* a process can vote YES or NO *)
  ∧ someFail ∈ [ Proc → BOOLEAN ]   (* initialization of failure detector *)
...
InitYes ≜  
  ...
  ∧ nRcvdYes = [ i ∈ Proc ↦ 0 ]    
  ∧ nRcvdNo = [ i ∈ Proc ↦ 0 ]  
  ∧ pc ∈ [ Proc → { "YES" } ]       (* all processes vote YES *)
  ∧ someFail ∈ [ Proc → BOOLEAN ]   (* initialization of failure detector *)
...
InitNo ≜  
  ...
  ∧ nRcvdYes = [ i ∈ Proc ↦ 0 ]
  ∧ nRcvdNo = [ i ∈ Proc ↦ 0 ]  
  ∧ pc ∈ [ Proc → { "NO" } ]        (* all processes vote NO *)
  ∧ someFail ∈ [ Proc → BOOLEAN ]   (* initialization of failure detector *)
...
TypeOK ≜ 
  ...
  ∧ nSntNoF ∈ 0‥N
  ∧ nSntYesF ∈ 0‥N    
  ∧ nRcvdYes ∈ [ Proc → 0‥(nSntYes + nSntYesF) ]
  ∧ nRcvdNo ∈ [ Proc → 0‥(nSntNo + nSntNoF) ] 
  ∧ pc ∈ [ Proc → Status ]
  ∧ someFail ∈ [ Proc → BOOLEAN ]      
   
(* No two correct processes decide differently. *)   
AgrrLtl ≜
  □(¬(∃ i ∈ Proc, j ∈ Proc :  pc[i] = "COMMIT" ∧ pc[j] = "ABORT"))
  
(* If some process votes NO, no process commits. *)  
AbortValidityLtl ≜
  (∃ i ∈ Proc : pc[i] = "NO") ⇒ □(∀ i ∈ Proc : pc[i] ≠ "COMMIT")
    
(* If every processes votes YES and no process is suspected, then no process crashes or aborts. *)    
CommitValidityLtl ≜
  (∀ i ∈ Proc : pc[i] = "YES" ∧ someFail[i] = FALSE) ⇒ □(∀ i ∈ Proc : pc[i] = "BYZ" ∨ pc[i] ≠ "ABORT")    
  
(* If no process crashes or is suspicious, then every process eventually commits. *)  
TerminationLtl ≜
  (□(∀ i ∈ Proc : pc[i] ≠ "BYZ" ∧ someFail[i] = FALSE)) ⇒ ◇(∀ i ∈ Proc : pc[i] = "COMMIT" ∨ pc[i] = "ABORT")      
